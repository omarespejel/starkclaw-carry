...
describe("ExtendedClient", () => {
  ...
  it("fetches market snapshot from live status/data envelope", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      jsonResponse({
        status: "OK",
        data: [
  ...
  it("fetches funding history from compact data envelope", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      jsonResponse({
        status: "OK",
        data: [
          { m: "ETH-USD", f: "0.000012", T: 1777777000000 },
          { m: "ETH-USD", f: "0.000013", T: 1777777600000 },
        ],
      }),
    );
  ...
  it("fetches wrapped user fee rates with API key", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      jsonResponse({
        status: "OK",
        data: [
